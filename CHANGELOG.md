...
### Fixed
* `AssembledTransaction.needsNonInvokerSigningBy()` and `signAuthEntries()` now see every signer node of a `SOROBAN_CREDENTIALS_ADDRESS_WITH_DELEGATES` entry (CAP-71), not just the top-level credentials. Previously `needsNonInvokerSigningBy()` reported only `signers[0]`, either an address that per the account's own policy may never need to sign (top-level unsigned by design, CAP-71-01) or nothing at all (top-level already signed, unsigned delegate never surfaced), and `signAuthEntries()` resolved an entry's signer via `getAddressCredentials()`, which never exposes a delegate address, so an entry needing a delegate's signature was silently skipped regardless of which delegate `address` targeted. `needsNonInvokerSigningBy()` is now documented as conservative for this credential type: it can still report a top-level node the account's policy doesn't actually require, so signing everyone it lists is always sufficient but not always necessary ([#1655](https://github.com/stellar/js-stellar-sdk/issues/1655), [#1672](https://github.com/stellar/js-stellar-sdk/pull/1672)).
* `signAuthEntries()` no longer silently re-signs an already-signed node when `address` matches one (added a missing `!signer.signed` check), and no longer corrupts a prior signer's signature during sequential multi-party signing: once any node on an entry is signed, every later `signAuthEntries()` call for the same entry now reuses the already-stored `signatureExpirationLedger` instead of computing a fresh one, since every signature on an entry commits to that one shared value and a later signer picking a different expiration silently invalidated an earlier signer's signature (real host rejection: `Error(Auth, InvalidAction)`, with no client-side error beforehand) ([#1672](https://github.com/stellar/js-stellar-sdk/pull/1672)).
* `signAuthEntries()`'s default authorizer now throws a clear `AssembledTransaction.Errors.UnsupportedDelegateSigner` when asked to sign for a contract-address (`C...`) delegate (a `Signer::Delegated`-style signer), instead of an opaque strkey error from deep inside signature verification — this path needs a `signatureScVal`, which the default authorizer can't produce; a custom `authorizeEntry` is still free to try. `forAddress` is now always passed through to a custom `authorizeEntry`, and `signAuthEntries()` verifies afterward that the delegate node it names actually got signed, throwing `AssembledTransaction.Errors.AuthorizeEntryMissingForAddress` if not, instead of silently leaving it unsigned until the network rejects it later ([#1672](https://github.com/stellar/js-stellar-sdk/pull/1672)).
...
* Updated the production `axios` dependency from `1.18.0` to `1.20.0` ([#1712](https://github.com/stellar/js-stellar-sdk/pull/1712)).
...
* `xdr.Int32`, `xdr.Uint32`, `xdr.Int64` and `xdr.Uint64` report their XDR type name from `.name`, instead of the internal `"Shim"` ([#1682](https://github.com/stellar/js-stellar-sdk/pull/1682)).
...
* `HorizonApi.TransactionFailedExtras`'s `result_codes.operations` is now optional (`operations?: string[]`). Horizon omits the field when a transaction fails a transaction-level check (e.g. `tx_bad_seq`) and no operations were evaluated, so the type now matches the wire format. Under `strictNullChecks`, unguarded reads of the raw response (`extras.result_codes.operations.map(...)`) no longer compile; guard them, or use `TransactionFailedError.getResultCodes()`, which normalizes the omitted field to `[]` ([#1527](https://github.com/stellar/js-stellar-sdk/issues/1527)).
...
### Changed
- `scValToNative` converts an `scvExecutableTag` to its tag: a string when the bytes are valid UTF-8, otherwise the raw bytes (same rule as `scvString`) ([#1577](https://github.com/stellar/js-stellar-sdk/pull/1577)).
...
- `contract.Client.txFromJSON` is now `txFromJson`, and generated bindings' `fromJSON` is now `fromJson`, matching the `toJson`/`fromJson` naming used across the XDR layer. Both keep a deprecated alias, so existing calls still work ([#1422](https://github.com/stellar/js-stellar-sdk/pull/1422)).
...
### Fixed
* The legacy `new xdr.SomeUnion(discriminant, value)` form throws a `TypeError` naming the arm factory to call instead, on all 115 generated union types ([#1640](https://github.com/stellar/js-stellar-sdk/issues/1640), [#1658](https://github.com/stellar/js-stellar-sdk/pull/1658)). Unions are abstract base classes exported as values, and `abstract` is erased at runtime, so the pre-v17 call built a base instance that silently discarded both arguments — `Object.keys(m)` was `[]` and `m.type` was `undefined`. It only failed once something serialized it, with `TypeError: this.toXdrObject is not a function` thrown from inside the SDK, naming neither the union nor the call that created it; a `new` in a module-level constant surfaced as the whole module failing at import. TypeScript already rejected the form (`TS2511: Cannot create an instance of an abstract class`), so this reaches plain JavaScript and TypeScript run without a type-check pass. `xdr.Int64` / `xdr.Uint64` / `xdr.Int32` / `xdr.Uint32` have guarded their equivalent legacy form since 17.0.0-rc.1; unions now match.
...
- `contract.Signer`: an interface pairing an `address` with the SEP-43 `signTransaction` and optional `signAuthEntry` methods, plus `contract.KeypairSigner`, a `Keypair`-backed implementation. The `signTransaction` and `signAuthEntry` options — on `ClientOptions`, `MethodOptions`, and `AssembledTransaction`'s `sign` / `signAndSend` / `signAuthEntries` — now accept a `Signer` or a bare `Keypair` in addition to a callback. Adds the `contract.SignTransactionLike` and `contract.SignAuthEntryLike` types. When `signAuthEntries` gets a `Signer` or `Keypair`, its default target `address` is now the signer's own address rather than `publicKey`. Existing callbacks work unchanged; one type-only caveat: the option fields are no longer plain function types, so derive callback shapes from `contract.SignTransaction` / `contract.SignAuthEntry` instead of the option field ([#1567](https://github.com/stellar/js-stellar-sdk/pull/1567), closes [#1462](https://github.com/stellar/js-stellar-sdk/issues/1462) and [#1063](https://github.com/stellar/js-stellar-sdk/issues/1063)).
...
* Removed adding `resourceFee` in `assembleTransaction` as it's now handled by `TransactionBuilder.build()` ([#1343](https://github.com/stellar/js-stellar-sdk/pull/1343)).
