    ...
    it("fetches transactions successfully", function (done) {
        ...
        this.server
            ...
            .then((resp) => {
                ...
                expect(resp.transactions[0].returnValue).to.eql(new XdrLargeInt("u64", 1234).toScVal());
                expect(resp.transactions[1].returnValue).to.eql(new XdrLargeInt("u64", 1235).toScVal());
                done();
            })
            .catch((err) => done(err));
    });
