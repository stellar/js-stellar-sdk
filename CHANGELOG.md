...
* **`Memo.text` no longer accepts a plain `number[]`** — pass `new Uint8Array(arr)` instead ([#1457](https://github.com/stellar/js-stellar-sdk/issues/1457)). This is a swap rather than a pure narrowing: through 16.2.0 `Memo.text` took a `string`, a plain array, or a `Buffer` but *rejected* a bare `Uint8Array`, which is now the canonical byte input. A plain array is the only input actually lost — `Int8Array`, `Uint16Array`, and other non-`Uint8Array` typed arrays were rejected in 16.2.0 too, and `Memo.hash` / `Memo.return` never took plain arrays and are unchanged. One case to watch: `Memo.text([])` was a valid zero-byte memo in 16.2.0 and now throws. The thrown message is unchanged (`Expects string or Uint8Array, max 28 bytes`), so error-matching code still matches; see [`docs/UINT8ARRAY_MIGRATION.md`](./docs/UINT8ARRAY_MIGRATION.md) § 3.
...
* Rebuilding the XDR layer changed a few SDK-level behaviors that don't involve typing `xdr.` yourself. Most of these fail silently, so they won't surface as compile errors ([#1422](https://github.com/stellar/js-stellar-sdk/pull/1422)):
  * [`scValToNative`](https://stellar.github.io/js-stellar-sdk/reference/core-soroban-primitives/#scvaltonative) returns a `Uint8Array` for an `scvString` / `scvSymbol` whose contents aren't valid UTF-8. It previously always returned a string, substituting U+FFFD — its byte-returning branch was unreachable. Guards like `typeof result === "string"` and calls like `result.startsWith(...)` are now data-dependent. The same applies to [`contract.Spec.scValToNative`](https://stellar.github.io/js-stellar-sdk/reference/contracts-client/#specscvaltonativescv-typedef) and [`contract.Spec.funcResToNative`](https://stellar.github.io/js-stellar-sdk/reference/contracts-client/#specfuncrestonativename-val_or_base64) for `Bytes` / `BytesN`, which return `Uint8Array`; those are generically typed, so TypeScript won't flag it.
