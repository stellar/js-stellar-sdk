...
const { StellarTomlResolver, STELLAR_TOML_MAX_SIZE } = StellarSdk.Federation;
...
`,
      ...
      StellarTomlResolver.resolve("acme.com").then((stellarToml) => {
...
`,
      ...
      StellarTomlResolver.resolve("acme.com", {
        allowHttp: true,
      }).then((stellarToml) => {
        expect(stellarToml.FEDERATION_SERVER).equals(
...
`,
    ...
    it("rejects when there was a connection error", function (done) {
      ...
      StellarTomlResolver.resolve("acme.com").should.be.rejected.and.notify(
        done,
      );
    });
    ...
    it("fails when response exceeds the limit", function (done) {
      ...
      let tempServer = http
        ...
        .listen(4444, () => {
          StellarTomlResolver.resolve("localhost:4444", {
            allowHttp: true,
          })
