...
### Fixed
* Horizon call builders now confine a resource identifier to one URL path segment. `accounts().accountId()`, `transactions().transaction()`, `operations().operation()`, `offers().offer()`, `claimableBalances().claimableBalance()`, `ledgers().ledger()`, `liquidityPools().liquidityPoolId()`, the `for*()` neighbor filters and `loadAccount()` percent-encode the identifier, so a `/` no longer opens a new path segment and an identifier of `.` or `..` throws a `BadRequestError` instead of resolving to a different endpoint. An empty or nullish identifier throws for the same reason, where it previously resolved to the collection endpoint and returned a page of records rather than the requested resource. Well-formed identifiers are unaffected, since no valid Stellar identifier contains `/`, `.`, `?` or `#` ([#XXXX](https://github.com/stellar/js-stellar-sdk/pull/XXXX)).
