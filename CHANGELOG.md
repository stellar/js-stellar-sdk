...
- `Operation.createCustomContract` accepts a CAP-85 `externalRef` — either `{owner, tag}` or an `xdr.ContractExecutableExternalRef` — as an alternative to `wasmHash`; exactly one of the two must be given. Adds the `ExternalExecutableRef` type (ported from [#1665](https://github.com/stellar/js-stellar-sdk/pull/1665)).
- `contract.Client.deploy` accepts the same `externalRef` option and resolves the reference on-chain to fetch the contract's spec; generated bindings' `deploy` emits the option too (ported from [#1665](https://github.com/stellar/js-stellar-sdk/pull/1665)).
- `rpc.Server.getExternalRefWasmHash(ref)`: resolves a CAP-85 external executable reference to the 32-byte Wasm hash it names, by reading the owner contract's persistent tag entry. `getContractWasmByContractId` and the bindings generator resolve external-ref contracts through it automatically.

### Fixed
- `rpc.Server.getContractData` no longer reports transport failures (rate limits, 5xx, network errors) as a missing entry: those now propagate to the caller, and the `{code: 404}` rejection is reserved for an entry that is actually absent (ported from [#1679](https://github.com/stellar/js-stellar-sdk/pull/1679)).
- `Keypair.verify` and `verifyMessage` now throw a `TypeError` for an argument of an unaccepted type (a hex/base64 string, a plain number array, a `DecoratedSignature`) instead of reporting it as an invalid signature; a well-formed but wrong signature still returns `false`. `authorizeEntry` validates its signing callback's result — the naked-bytes, `{signature, publicKey}`, and `{signatureScVal}` forms — and throws a `TypeError` naming the actual mistake instead of surfacing a misleading downstream error (ported from [#1649](https://github.com/stellar/js-stellar-sdk/pull/1649)).
