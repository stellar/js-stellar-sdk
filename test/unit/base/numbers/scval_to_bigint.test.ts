...
import { scValToBigInt } from "../../../../src/base/numbers/index.js";
...
describe("scValToBigInt — scvU64", () => {
  it("converts 0", () => {
    const scv = xdr.ScVal.scvU64(xdr.Uint64(0));
    expect(scValToBigInt(scv)).toBe(0n);
  });

  it("converts 1", () => {
    const scv = xdr.ScVal.scvU64(xdr.Uint64(1));
    expect(scValToBigInt(scv)).toBe(1n);
  });

  it("converts large value", () => {
    const scv = xdr.ScVal.scvU64(xdr.Uint64("18446744073709551615"));
    expect(scValToBigInt(scv)).toBe(18446744073709551615n);
  });

  it("converts mid-range value", () => {
    const scv = xdr.ScVal.scvU64(xdr.Uint64(1000000n));
...
describe("scValToBigInt — scvI64", () => {
  it("converts 0", () => {
    const scv = xdr.ScVal.scvI64(xdr.Int64(0));
    expect(scValToBigInt(scv)).toBe(0n);
  });

  it("converts positive", () => {
    const scv = xdr.ScVal.scvI64(xdr.Int64(999));
  ...
  it("converts max i64", () => {
    const scv = xdr.ScVal.scvI64(xdr.Int64("9223372036854775807"));
    expect(scValToBigInt(scv)).toBe(9223372036854775807n);
  });

  it("converts min i64", () => {
    const scv = xdr.ScVal.scvI64(xdr.Int64("-9223372036854775808"));
...
describe("scValToBigInt — scvTimepoint", () => {
  it("converts 0", () => {
    const scv = xdr.ScVal.scvTimepoint(xdr.Uint64(0));
    expect(scValToBigInt(scv)).toBe(0n);
  });

  it("converts a realistic timestamp", () => {
    const scv = xdr.ScVal.scvTimepoint(xdr.Uint64(1443571200n));
    expect(scValToBigInt(scv)).toBe(1443571200n);
  });

  it("converts max u64 timepoint", () => {
    const scv = xdr.ScVal.scvTimepoint(xdr.Uint64("18446744073709551615"));
    expect(scValToBigInt(scv)).toBe(18446744073709551615n);
  });
});
...
describe("scValToBigInt — scvDuration", () => {
  it("converts 0", () => {
    const scv = xdr.ScVal.scvDuration(xdr.Uint64(0));
    expect(scValToBigInt(scv)).toBe(0n);
  });

  it("converts small duration", () => {
    const scv = xdr.ScVal.scvDuration(xdr.Uint64(3600n));
    expect(scValToBigInt(scv)).toBe(3600n);
  });

  it("converts large duration", () => {
    const scv = xdr.ScVal.scvDuration(xdr.Uint64("18446744073709551615"));
    expect(scValToBigInt(scv)).toBe(18446744073709551615n);
  });
});
...
describe("scValToBigInt — scvU128", () => {
  ...
  it("converts max u128", () => {
    ...
    const scv = xdr.ScVal.scvU128(
      new xdr.Uint128Parts({
        lo: xdr.Uint64(maxU64),
        hi: xdr.Uint64(maxU64),
      }),
    );
...
describe("scValToBigInt — scvU256", () => {
  ...
  it("converts max u256", () => {
    ...
    const scv = xdr.ScVal.scvU256(
      new xdr.Uint256Parts({
        loLo: xdr.Uint64(maxU64),
        loHi: xdr.Uint64(maxU64),
        hiLo: xdr.Uint64(maxU64),
        hiHi: xdr.Uint64(maxU64),
      }),
    );
...
describe("scValToBigInt — scvI256", () => {
  ...
  it("converts negative value", () => {
    ...
    const scv = xdr.ScVal.scvI256(
      new xdr.Int256Parts({
        loLo: xdr.Uint64(loLo.toString()),
        loHi: xdr.Uint64(maxU64Str),
        hiLo: xdr.Uint64(maxU64Str),
        hiHi: xdr.Int64(-1),
      }),
    );
    expect(scValToBigInt(scv)).toBe(-100n);
...
describe("scValToBigInt — error cases", () => {
  ...
  it("throws on scvBytes", () => {
    expect(() =>
      scValToBigInt(
        xdr.ScVal.scvBytes(new xdr.ScBytes(Uint8Array.from([1, 2, 3]))),
      ),
    ).toThrow(TypeError);
  });
...
describe("scValToBigInt — round-trip consistency", () => {
  ...
  it("u64 and i64 produce the same bigint for the same positive value", () => {
    const u = scValToBigInt(xdr.ScVal.scvU64(xdr.Uint64(100)));
    const i = scValToBigInt(xdr.ScVal.scvI64(xdr.Int64(100)));
    expect(u).toBe(i);
  });

  it("u128 value 42 matches u64 value 42", () => {
    const from64 = scValToBigInt(xdr.ScVal.scvU64(xdr.Uint64(42)));
  ...
  it("i128 negative matches i64 negative for small values", () => {
    const from64 = scValToBigInt(xdr.ScVal.scvI64(xdr.Int64(-50)));
  ...
  it("timepoint and duration with same value produce same bigint", () => {
    const tp = scValToBigInt(xdr.ScVal.scvTimepoint(xdr.Uint64(999)));
    const dur = scValToBigInt(xdr.ScVal.scvDuration(xdr.Uint64(999)));
    expect(tp).toBe(dur);
  });
});
