...
* These `TypeError`s are thrown synchronously from `call()` and `stream()` rather than as a rejected promise, which matches the existing `Too many filters specified` error, so a caller that only attaches `.catch()` needs a `try`/`catch`. Two methods differ: `loadAccount()` is `async`, so it rejects and `.catch()` does work, and `liquidityPools().liquidityPoolId()` throws from the builder method, as it always has ([#1702](https://github.com/stellar/js-stellar-sdk/pull/1702)).
