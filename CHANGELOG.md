...
### Fixed
* The legacy `new xdr.SomeUnion(discriminant, value)` form throws a `TypeError` naming the arm factory to call instead, on all 115 generated union types ([#1640](https://github.com/stellar/js-stellar-sdk/issues/1640)). Unions are abstract base classes exported as values, and `abstract` is erased at runtime, so the pre-v17 call built a base instance that silently discarded both arguments — `Object.keys(m)` was `[]` and `m.type` was `undefined`. It only failed once something serialized it, with `TypeError: this.toXdrObject is not a function` thrown from inside the SDK, naming neither the union nor the call that created it; a `new` in a module-level constant surfaced as the whole module failing at import. TypeScript already rejected the form (`TS2511: Cannot create an instance of an abstract class`), so this reaches plain JavaScript and TypeScript run without a type-check pass. `xdr.Int64` / `xdr.Uint64` / `xdr.Int32` / `xdr.Uint32` have guarded their equivalent legacy form since 17.0.0-rc.1; unions now match.
