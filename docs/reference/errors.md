...
`xdr.TransactionResult`, which are otherwise nested deep inside the
...
class TransactionFailedError extends BadResponseError {
  ...
  stack?: string;
  getResponse(): { data?: ErrorResponseData; status?: number; statusText?: string; url?: string };
  getResultCodes(): { operations: string[]; transaction: TransactionFailedResultCodes };
  getTransactionResult(): TransactionResult | null;
}
```
...
**Source:** [src/errors/transaction_failed.ts:30](https://github.com/stellar/js-stellar-sdk/blob/main/src/errors/transaction_failed.ts#L30)

### `new TransactionFailedError(message, response)`
...
### `TransactionFailedError.stackTraceLimit`
...
### `TransactionFailedError.captureStackTrace(targetObject, constructorOpt)`
...
### `TransactionFailedError.prepareStackTrace(err, stackTraces)`
...
### `transactionFailedError.message`
...
### `transactionFailedError.response`
...
### `transactionFailedError.getResponse()`
...
### `transactionFailedError.getResultCodes()`
...
The `result_codes` object from the response's `extras`.

**Source:** [src/errors/transaction_failed.ts:41](https://github.com/stellar/js-stellar-sdk/blob/main/src/errors/transaction_failed.ts#L41)

### `transactionFailedError.getTransactionResult()`

Decodes and returns the `xdr.TransactionResult` from the response's
