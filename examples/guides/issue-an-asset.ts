...
const line = distributorAccount.balances.find(
  (b) =>
    "asset_code" in b &&
    b.asset_code === "ASTRO" &&
    b.asset_issuer === issuer.publicKey(),
);
console.log("ASTRO balance:", line?.balance);
// #endregion full

if (!line) {
  throw new Error("guide distributor has no ASTRO trustline from the issuer");
}
if (line.balance !== "1000.0000000") {
  throw new Error(`guide distributor holds ${line.balance} ASTRO, not 1000`);
}
