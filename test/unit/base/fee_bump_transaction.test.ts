...
describe("FeeBumpTransaction", () => {
  ...
  it("constructs a FeeBumpTransaction object from a TransactionEnvelope", () => {
    ...
    expect(uint8ArrayToString(innerTransaction.memo.value as Uint8Array)).toBe(
      "Happy birthday!",
    );
  ...
  it("signs using hash preimage", () => {
    ...
    expectBytesToBeEqual(
      new Uint8Array(decoratedSignature.signature.value),
      preimage,
    );
    expectBytesToBeEqual(
      new Uint8Array(decoratedSignature.hint.value),
  ...
  it("adds signature correctly", () => {
    ...
    expect(
      feeSource.verify(
        ...
        new Uint8Array(addedSignature.signature.value),
      ),
    ).toBe(true);
    ...
    expectBytesToBeEqual(
      new Uint8Array(signedSignature.signature.value),
      new Uint8Array(addedSignature.signature.value),
    );

    expectBytesToBeEqual(
      new Uint8Array(signedSignature.hint.value),
      new Uint8Array(addedSignature.hint.value),
    );

    expectBytesToBeEqual(addedSignatureTx.hash(), transaction.hash());
  });
