...
### Fixed
* Horizon call builders now confine a resource identifier to one URL path segment. `accounts().accountId()`, `transactions().transaction()`, `operations().operation()`, `offers().offer()`, `claimableBalances().claimableBalance()`, `ledgers().ledger()`, the `for*()` neighbor filters and `loadAccount()` percent-encode the identifier, so a `/` in it no longer opens a new path segment, and an empty or `.`/`..` identifier throws a `TypeError` instead of resolving to a different endpoint — `accountId("")` previously reached the collection endpoint and returned a page of records. Methods that already validate their argument, such as `liquidityPools().liquidityPoolId()`, keep their existing `TypeError`. Well-formed identifiers are unaffected, since no valid Stellar identifier contains `/`, `.`, `?` or `#` ([#1702](https://github.com/stellar/js-stellar-sdk/pull/1702)).
