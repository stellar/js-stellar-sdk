...
### Fixed
* Horizon call builders now confine a resource identifier to one URL path segment, so a `/` in it no longer opens a new segment. An identifier that is empty, `.`, `..`, or not a `string`/`number`/`bigint` throws a `TypeError` instead of re-pointing the request — `forAccount([".."])` previously reached the network-wide `/transactions` feed. A decimal `number` or `bigint` identifier still works. Affects `accountId()`, `transaction()`, `operation()`, `offer()`, `claimableBalance()`, `ledger()`, `loadAccount()` and the `for*()` neighbor filters. Well-formed identifiers are unaffected ([#1702](https://github.com/stellar/js-stellar-sdk/pull/1702)).
* The Horizon call builder `TypeError`s above throw synchronously from `call()` and `stream()` rather than as a rejected promise, matching the existing `Too many filters specified` error, so a caller that only attaches `.catch()` needs a `try`/`catch` ([#1702](https://github.com/stellar/js-stellar-sdk/pull/1702)).
