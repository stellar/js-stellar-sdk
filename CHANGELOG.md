...
* The `xdr` namespace is rebuilt on `@stellar/js-xdr` v5, and **every XDR value now has a different API** ([#1422](https://github.com/stellar/js-stellar-sdk/pull/1422)). The wire format is unchanged: bytes and base64 written by older SDKs still decode, and vice versa. Any code that reads or builds `xdr.*` values must be updated. The main shifts:
  ...
  * Primitives are plain JS values. Integers are `number` or `bigint` instead of class wrappers, `LargeInt` subclasses are gone, and fields are `readonly`. Byte fields split in two: an inline `opaque[N]` / `opaque<N>` (or `uint256`) is a raw `Uint8Array`, while the fourteen capitalized byte typedefs (`Hash`, `Signature`, `ScBytes`, …) are wrappers holding the bytes at `.value` / `.toBytes()`. A wrapper has no `.length` and `Array.from()` on one returns `[]`, so compare with `.equals()`.
