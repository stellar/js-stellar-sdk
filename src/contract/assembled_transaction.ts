    ...
    return [
      ...new Set(
        (rawInvokeHostFunctionOp.auth ?? [])
          .filter((entry: SorobanAuthorizationEntry) => {
            if (entry.credentials.type !== "sorobanCredentialsAddress") {
              return false;
            }
            return (
              includeAlreadySigned ||
              entry.credentials.value.signature.type === "scvVoid"
            );
          })
          .map((entry: SorobanAuthorizationEntry) => {
            const creds = entry.credentials as SorobanCredentials & {
              type: "sorobanCredentialsAddress";
              value: { address: ScAddress };
            };
            return Address.fromScAddress(creds.value.address).toString();
          }),
      ),
    ];
  };
