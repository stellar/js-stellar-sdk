...
SERVER_URLS.forEach((serverUrl) => {
  describe(`with server URL: ${serverUrl}`, () => {
    ...
    it("server.accounts().accountId('fooAccountId')", async () => {
      const testResult = prepareAxiosMock("/accounts/fooAccountId");
