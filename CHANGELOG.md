...
### Fixed
* `AssembledTransaction.needsNonInvokerSigningBy()` and `signAuthEntries()` now see every signer node of a `SOROBAN_CREDENTIALS_ADDRESS_WITH_DELEGATES` entry (CAP-71), not just the top-level credentials. Previously `needsNonInvokerSigningBy()` reported only `signers[0]`, either an address that per the account's own policy may never need to sign (top-level unsigned by design, CAP-71-01) or nothing at all (top-level already signed, unsigned delegate never surfaced), and `signAuthEntries()` resolved an entry's signer via `getAddressCredentials()`, which never exposes a delegate address, so an entry needing a delegate's signature was silently skipped regardless of which delegate `address` targeted ([#1655](https://github.com/stellar/js-stellar-sdk/issues/1655), [#1672](https://github.com/stellar/js-stellar-sdk/pull/1672)).
* `signAuthEntries()`'s default `authorizeEntry` callback now verifies against the `signerAddress` a `signAuthEntry` callback reports, instead of unconditionally verifying against `target` (the address that was asked to sign). `target` names who was requested, not who actually signed: a delegate whose real signing key differs from the delegate node's own address (for example a smart-account delegate proxying to an underlying Ed25519 key) previously failed with `signature doesn't match payload` even when the reported signature was genuinely valid, because the wrong public key was handed to the verification step. Note for callers of `authorizeEntry` directly with a custom, loosely-typed `SigningCallback`: the callback `signAuthEntries()` hands to a custom `authorizeEntry` now resolves to `{ signature, publicKey }` instead of a naked `Uint8Array`; this was already type-sanctioned but is a real runtime shape change for an implementation that duck-types its input ([#1672](https://github.com/stellar/js-stellar-sdk/pull/1672)).
...
* `signAuthEntries()` now throws a clear `AssembledTransaction.Errors.UnsupportedDelegateSigner` when asked to sign for a contract-address (`C...`) delegate (a `Signer::Delegated`-style signer), instead of an opaque strkey error from deep inside signature verification — this path needs a `signatureScVal`, which nothing in `signAuthEntries()` can produce yet; use a custom `authorizeEntry` for that case. It also now throws a clear `AssembledTransaction.Errors.AuthorizeEntryMissingForAddress` when a custom `authorizeEntry` passed in doesn't declare enough parameters to receive the `forAddress` (5th) argument needed to target a delegate specifically, instead of silently writing the signature to the entry's top-level node and leaving the actual delegate unsigned until the network rejects it later ([#1672](https://github.com/stellar/js-stellar-sdk/pull/1672)).
* `authorizeEntry`'s deprecated bare-`Uint8Array`-returning `SigningCallback` form now verifies the signature against `forAddress` when the caller passed one, instead of always the entry's own top-level address — matching what the object-returning (`{ signature, publicKey }`) and `signAuthEntries()`-internal forms already did correctly. A signer targeting a CAP-71 delegate or any non-top-level address via this specific callback shape previously failed with `signature doesn't match payload` even for a correctly-produced signature ([#1683](https://github.com/stellar/js-stellar-sdk/issues/1683)).
